...
import GaussianHilbert.PolynomialDensity
...
1. (analytic) Multivariate polynomials are dense in `L²(μ)` for any sub-Gaussian
   ...
   `GaussianHilbert.polynomial_dense_L2_of_subGaussian` and applied
   to `stdGaussianFin n` via
   `GaussianHilbert.isSubGaussianMeasure_pi_gaussianReal`.
...
theorem hermiteMulti_dense {n : ℕ} (f : (Fin n → ℝ) → ℝ)
  ...
  haveI : MeasureTheory.IsProbabilityMeasure (stdGaussianFin n) := by
    ...
    infer_instance
  have hSG :
      GaussianHilbert.IsSubGaussianMeasure (stdGaussianFin n) :=
    GaussianHilbert.isSubGaussianMeasure_pi_gaussianReal n
  obtain ⟨p, hp⟩ :=
