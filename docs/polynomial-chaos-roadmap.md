...
| File | Lines | Status |
|---|---|---|
| `GaussianHilbert/HermitePolynomials.lean` | ~460 | **Proved**: `hermiteMulti_orthogonality`, `hermiteMulti_l2_pos`, `hermiteMulti_dense`. The density theorem rests on one external axiom in gaussian-field (`polynomial_dense_L2_of_subGaussian`, Janson Thm 2.6). |
| `GaussianHilbert/WienerChaos.lean` | ~530 | **Proved**: `wienerChaos n k` def, `wienerChaos_orthogonal`, `chaosProjection_sum_eq_of_mem_wienerChaosLE`, full chaos arithmetic, and `wienerChaos_isHilbertSum` (the Hilbert-sum decomposition `L²(γ) = ⊕̂ₖ Hₖ`, derived from `hermiteMulti_dense` via `IsHilbertSum.mkInternal`). The legacy `wienerChaos_isInternalDirectSum` axiom statement was *strictly stronger* than the true theorem (would require finite chaos expansions) and has been replaced. |
| `GaussianHilbert/OUEigenfunctions.lean` | ~540 | **Proved**: `ouGenerator_hermiteMultiEval` (1D + multivariate eigenfunction property `L H_α = -|α| H_α`). Three placeholder axioms remain for the OU semigroup operator (`ouSemigroupAct`) and its action on chaos pieces (`ouSemigroupAct_eq_smul_of_mem_wienerChaos`, `ouSemigroupAct_eLpNorm_hypercontractive`) — these will be discharged once markov-semigroups `Diffusion/OrnsteinUhlenbeck.lean` is filled in. |
| `GaussianHilbert/PolynomialChaosConcentration.lean` | ~630 | **Proved**: `bonami_nelson_chaos`, `bonami_nelson_chaosLE`, `polynomial_chaos_concentration`. Axiom footprint per `#print axioms`: the three OU placeholder axioms above (Lean built-ins aside). |
...
`GaussianHilbert/OUEigenfunctions.lean` (consumed by the Bonami-Nelson and
...
4. Derive Bonami-Nelson on $\mathcal H_k$ from the OU eigenfunction
   ...
   markov-semigroups `Abstract/Hypercontractivity.lean`.
...
`GaussianHilbert/`, parallel to `Diffusion/`.

### `GaussianHilbert/HermitePolynomials.lean` — multivariate Hermite
...
### `GaussianHilbert/WienerChaos.lean` — chaos decomposition
...
### `GaussianHilbert/OUEigenfunctions.lean` — H_k as eigenspace of L
...
### `GaussianHilbert/PolynomialChaosConcentration.lean` — Janson 5.10
...
- markov-semigroups `Abstract/Hypercontractivity.lean` — `IsHypercontractive`, Gross's
  equivalence (LSI ↔ HC), abstract `semigroup_lp_improvement`. Used
  by `bonami_nelson_chaos`.
- markov-semigroups `Diffusion/OrnsteinUhlenbeck.lean` — **still a 45-line skeleton.** The
  three placeholder axioms in `GaussianHilbert/OUEigenfunctions.lean` will be
...
1. **Fill in markov-semigroups `Diffusion/OrnsteinUhlenbeck.lean`** with the OU semigroup,
...
2. **Bonami-Nelson hypercontractivity proof** (`ouSemigroupAct_eLpNorm_hypercontractive`),
   ...
   markov-semigroups `Abstract/Hypercontractivity.gross_lsi_implies_hypercontractive` axiom
