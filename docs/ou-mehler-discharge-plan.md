...
- the 4 BGL Ch. 2 textbook axioms in markov-semigroups `Instances/WorkInProgress/Euclidean.lean` (1D Mehler-kernel facts)
...
These are the three placeholder axioms in `GaussianHilbert/OUEigenfunctions.lean`.
...
Create `GaussianHilbert/MehlerKernel.lean`:
...
Same file or `GaussianHilbert/MehlerSemigroup.lean`:
...
Either a new file `GaussianHilbert/MehlerHermite.lean` or extension of
`GaussianHilbert/OUEigenfunctions.lean`:
...
Create `GaussianHilbert/BakryEmeryInstance.lean`:
...
/-- The standard Gaussian on `Fin n → ℝ` as a Bakry-Émery space with
    ...
    - **semigroup_l2_decay_bound, semigroup_l2_sq_hasDerivWithinAt,
      ...
      markov-semigroups `Instances/WorkInProgress/Euclidean.lean`; the n-D version becomes
...
In `GaussianHilbert/OUEigenfunctions.lean`, replace the three
axioms:
...
markov-semigroups `Abstract/Hypercontractivity.lean` is in terms of the abstract
