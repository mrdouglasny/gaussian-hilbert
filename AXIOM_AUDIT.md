...
| `mehlerOp_eq_ouSemigroupAct` (agreement) | [`GaussianHilbert/OUEigenfunctions.lean`](GaussianHilbert/OUEigenfunctions.lean) (after `ouSemigroupAct_eq_smul_of_mem_wienerChaos`) | 2026-05-11 | Janson Thm 4.4; BGL §2.7 | Closed-kernel + density: `A := coord ∘ mehlerOp − chaosDiagCLM ∘ coord` vanishes on every `wienerChaos n k` by the eigenvalue identities; hence on the closure of `⨆ k`, which is `⊤` via the new `wienerChaos_iSup_topologicalClosure_eq_top`. ~70-line proof. Pre-requisite for the spatial (Mehler-integral) route to hypercontractivity. | future `ouSemigroupAct_eLpNorm_hypercontractive` discharge (Route W / N). |

All four discharged theorems verify clean via `#print axioms` —
their axiom dependency closures contain only `propext`,
`Classical.choice`, `Quot.sound`. The chaos infrastructure
(`hermiteMulti_dense → wienerChaos_isHilbertSum → chaosCoordEquiv → ouSemigroupAct → mehlerOp_eq_ouSemigroupAct`)
